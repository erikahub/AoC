...
def findPair(opList: list):
    for x in range(100):
        for y in range(100):
            listCopy = opList.copy()
            listCopy[1]=x
            listCopy[2]=y
            i=0
            length = len(listCopy)
            while listCopy[i] != 99:
                if (length < listCopy[i] or length < listCopy[i+1] or length < listCopy[i+2] or length < listCopy[i+3]):
                    print('invalid index')
                    i+=4
                    break
                opcode = listCopy[i]
                if opcode == 1:
                    # print('pos(%i) equalling %i = pos(%i) + pos(%i) with i = %i' % (listCopy[i+3], listCopy[listCopy[i+3]], listCopy[i+1], listCopy[i+2], i))
                    # print('%i = %i + %i' % ( listCopy[listCopy[i+1]]+listCopy[listCopy[i+2]],  listCopy[listCopy[i+1]], listCopy[listCopy[i+2]]))
                    listCopy[listCopy[i+3]] = listCopy[listCopy[i+1]] + listCopy[listCopy[i+2]]
                elif opcode == 2:
                    # print('pos(%i) equalling %i = pos(%i) + pos(%i) with i = %i' % (listCopy[i+3], listCopy[listCopy[i+3]], listCopy[i+1], listCopy[i+2], i))
                    # print('%i = %i + %i' % ( listCopy[listCopy[i+1]]*listCopy[listCopy[i+2]],  listCopy[listCopy[i+1]], listCopy[listCopy[i+2]]))
                    listCopy[listCopy[i+3]] = listCopy[listCopy[i+1]] * listCopy[listCopy[i+2]]
